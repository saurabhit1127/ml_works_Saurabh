...
def read_matrix(filename, url=True):
    """
...
def multiple_deletion_node_np1(matrix, msr_threshold=300, alpha=1.2):
    """
    Multiple deletion node on matrix.
    Parameters
    ----------
...
def single_deletion_node_np1(matrix, rows, cols, msr_threshold=300):
    """
    Single deletion node on submatrix defined by rows and cols.
    Parameters
    ----------
...
def find_firefly(matrix, n_of_bicluster=100, msr_threshold=200, alpha=1.2):
    """
...
def find_biclusters_np(matrix, n_of_bicluster=100, msr_threshold=100, alpha=1.2):
    """
    Find biclusters in a given matrix.
    Parameters
    ----------
    ...
    biclusters = []
    for i in range(n_of_bicluster):
        rowsB, colsB = multiple_deletion_node_np1(
            matrixA, msr_threshold=msr_threshold, alpha=alpha)
        rowsC, colsC = single_deletion_node_np1(
            matrixA, rowsB, colsB, msr_threshold=msr_threshold)
        rowsD, colsD, invD, msr = node_addition_np(matrix, rowsC, colsC)
        print("Bicluster " + str(i) + "using firefly")
        biclusters.append(Bicluster(rowsD, colsD, invD, msr))
        matrixA = hide_bicluster_np(matrixA, rowsD, colsD, invD)
    return biclusters
...
def main():
    #data = read_matrix("/Users/saurabhghosh/Downloads/yeast.matrix",url=True)
    data = read_matrix("http://arep.med.harvard.edu/biclustering/yeast.matrix",url=True)
    ...
    #sns.clustermap(data)
    #sns.clustermap(data, metric="correlation", standard_scale=1)
    #sns.clustermap(data, metric="euclidean", standard_scale=1)
    ...
    data = clean(data)
    start = time.time()
    firefly=find_firefly(data,n_of_bicluster=5)
    end = (time.time() - start)
    #print(end, "seconds")
    best_firefly = sorted(firefly, key=lambda bicluster: bicluster.msr)[0]
    maxgen_msr=best_firefly.msr
    print("best firefly")
    print(best_firefly)
    plot_bicluster(data, best_firefly)

    biclusters = find_biclusters_np(data, n_of_bicluster=3)

    end = (time.time() - start)
    msr_value = sorted(msr_values, reverse=True)
    print("last bicluster masked", end, "seconds")

    best_bicluster = sorted(biclusters, key=lambda bicluster: bicluster.msr)[0]
    print("best bicluster using firefly")
    ...
    print(msr_value)
    for i in range(l):
        if (msr_value[i] <= maxgen_msr):

            plt.plot(i, msr_value[i], "g *")
        else:
            plt.plot(i, msr_value[i], "r o")
